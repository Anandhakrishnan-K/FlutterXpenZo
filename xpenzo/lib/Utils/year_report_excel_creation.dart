...
Future<void> saveExcelFile(
  ...
  excel.rename('Sheet1', year.format(inputList[0].key).toString());
  final sheet = excel[year.format(inputList[0].key).toString()];
  ...
  for (int i = 0; i < inputList.length; i++) {
    ...
    sheet
        ...
        .value = month.format(inputList[i].key);
    sheet
...
class CreateExcelReportYearButton extends StatefulWidget {
  final DateTime? currDate;
  const CreateExcelReportYearButton({super.key, this.currDate});

  @override
  State<CreateExcelReportYearButton> createState() =>
      _CreateExcelReportYearButtonState();
}

class _CreateExcelReportYearButtonState
    extends State<CreateExcelReportYearButton> {
  @override
